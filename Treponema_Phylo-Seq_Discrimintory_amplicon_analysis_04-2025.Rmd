...
Load dependencies
```{r}
packages.list <- c("ggplot2","dplyr","tidyverse","treeio","ggtree","phytools","readxl","ggnewscale","ape","adegenet","pegas","mmod","vcfR","hierfstat","network","ggnetwork","intergraph","igraph","magick", "seqinr","IRanges","ggbeeswarm","primerTree","taxize","cowplot","ggstance","ggforce","vegan","Polychrome","ggrastr","treemapify","treespace","reshape2","doMC", "pairsnp")
...
Figures_directory <- paste0(getwd(), "/figures_revision_12-2025/")

Ancillary_Figures_directory <- paste0(getwd(), "/ancillary_figures/")

Gono_data <- paste0(getwd(), "/Gono_data/")
```
...
TPAE.pinecone.10.file <- paste0(Data_input_directory,"TPAE.all.gubbins-filtered.SNPs.aln.pyjar.joint.tre.rPinecone.10.3__20220225.csv.gz")


#############################################
# Precalculated FST scores for all newly redefined sublineages
FST.sublin.combined.TPAE.file <- paste0(Data_input_directory,"FST.sublin.combined.TPAE_20220606.csv.gz")

#############################################
# PrimalScheme design files (outputs of primalscheme)
primalscheme.design.file <- paste0(Data_input_directory,"TPAE_discriminatory_PrimalScheme.primer.tsv.gz")

SS14.primer_binding.locations.file <- paste0(Data_input_directory,"TP.disc.primalscheme.primerset.684_20220613.primer-location-hits-SS14.tsv.gz")
...
in_silico_pcr.results.file <- paste0(Data_input_directory,"in_silico_pcr.amplicon.recovery_list.tsv.gz")
...
COBS661K_window.search.23S.file <- paste0(Data_input_directory,"23S_Nichols_2-operons1.71bp-windows.20220623.fa_1_COBS-661K_results_table.txt.gz")

COBS661K.metadata.file <- paste0(Data_input_directory,"Blackwell2018__File2_taxid_lineage_661K.txt.gz")

# Primalscheme 23S design
primalscheme.design.23S.file <- paste0(Data_input_directory,"23S_scheme_primalscheme.primer.tsv")

primer.blast.23s.output.all.file <- paste0(Data_input_directory,"23S_o1_Nich_variable-site-masking.20220624.primerblast.searches.tsv.gz")


#############################################
# South Africa Validation - MinION QC data 
TP20230502SA.readLength.filt.file <- paste0(Data_input_directory,"SouthAfrica_TP_Discriminatory_SA1+2+3-all_read-lengths_filt.tsv.gz")

TP20230502SA.readLength.unfilt.file <- paste0(Data_input_directory,"SouthAfrica_TP_Discriminatory_SA1+2+3-all_read-lengths_unfilt.tsv.gz")

TP20230502SA.region.depth.file <- paste0(Data_input_directory,"TP_Discriminatory.SA1+SA2+SA3_filtered.depth_redo-2023-12-01.tsv.gz")
...
# reanalysis of original data using ONTAP nextflow pipeline
SA_ONT_R9_variants_ONTAP_repeat_calls.file <- paste0(Data_input_directory, "ONTAP_R9_SA1+2+3_allsites.merge.vcf") 
...
# Ancestrally reconstructed tree, with N sites filled based on reconstruction
revised_candidate.amplicon.SNP.seqs.alignment.pyjar.tree.filename <- paste0(Data_input_directory,"TPAE-selected-sites_n174sites__revised_amplicons.608dataset.20240528.pyjar.joint.tre")
...
#############################################
# Analysis of MLST calls
TPAE_dataset.mlst_calls.file <- paste0(Data_input_directory,"Phylo-Plex_dataset__MLST_Method_Comparisons__2025-04-29.xlsx")


#############################################
# Taouk Gonorrhoea analysis
Taouk_gono.metadata.file <- paste0(Gono_data,"Taouk_supdata_41467_2024_52343_MOESM4_ESM.xlsx")
Taouk_gono.processed.vcf.file <- paste0(Gono_data,"clean.SNPs.vcf.gz_processed_spread.tsv.gz")
Taouk_gono.FST.file <- paste0(Gono_data,"clean.SNPs.vcf.gz_processed_spread.parallel.FST.scoring.csv.gz")
Taouk_gono.processed.vcf.melt_in.phycons.file <- paste0(Gono_data,"clean.SNPs.vcf.gz_processed.tsv.gz_filtered_phycons.29396.tsv.gz")
Gono_Phycon_tree.file <- paste0(Gono_data,"Taouk_filtered_phycons.IQtree.tre")
Gono_WGS_tree.file <- paste0(Gono_data,"Taouk_snippy_clean.SNPs.aln.IQtree.tre")
Taouk_clean.SNPs_phycons_fasta.file <- paste0(Gono_data, "clean.SNPs.vcf.gz_processed.tsv.gz_filtered_phycons.fa") # needs to be gunzipped
...
for (current.count in seq(14,1)){
  # within loop, create dataset based on how many pops each region is discriminatory for (many = better)
...
#ggsave(plot=p.positional_disc_sites_clustered.site.network_by_sublin.both_ellipses__combi, paste0(Figures_directory,"Figure2__DiscSites_clustered.site.network_ellipses.",format(Sys.Date(),"%Y%m%d"),".pdf"), units='mm', width=210, height=220, device='pdf', dpi=400)
...
COBS661K_window.search.23S %>%
  filter(query == "23S__235035-235106") %>%
  group_by(major_species) %>%
  summarise(Species_count=n()) %>%
  arrange(desc(Species_count))

COBS661K_window.search.23S %>%
  #filter(major_species == "Treponema pallidum") %>%
  filter(grepl("Treponema", major_species)) %>%
  group_by(query) %>%
  summarise(Treponema_query_count=n()) %>%
  arrange(desc(Treponema_query_count))
...
```{r}
# mytree1= WGS.ML.tree.ult
# mytree2= revised_candidate.amplicon.SNP.seqs.alignment.tree
# mymetadata=data.frame(Sample_Name=TPAE.pinecone.10$Taxa, TPAE.pinecone=TPAE.pinecone.10$TPA.TPE.scheme, stringsAsFactors=F)
# colorscheme=TPA.TPE.scheme.cols
# scale_difference = 1750
# centre_separation = 250
# pointsize=1
# left_label_offset=0.15
# right_label_offset=0.9
# left_ladder=F
# right_ladder=F
# line_alpha=0.5
# point_alpha=0.15
# untangle=F # note, this can slow the code down substantially for big trees

plot_tanglegram <- function(mytree1, mytree2, mymetadata, colorscheme=TPA.TPE.scheme.cols, scale_difference=2000, centre_separation=200, pointsize=1, left_label_offset=0.15, right_label_offset=0.9, left_ladder=F, right_ladder=F, line_alpha=0.5, point_alpha=0.15, untangle=F){
  # Extract color variable name (1)
  colvar1.name <- colnames(mymetadata)[2]
  colnames(mymetadata) <- c("Sample_Name",colvar1.name)
  # Prepare color scheme
  colorscheme.temp <- colorscheme
  colnames(colorscheme.temp) <- c("variable","color")
  
  # untangle tips (to improve alignment without changing topology)
  if (untangle==T){
    mytree1 <- ape::ladderize(mytree1, right = F)
    mytree2 <- ape::ladderize(mytree2, right = F)
    untangled.object.temp <- phytools::untangle(c(mytree1, mytree2),  method = "stepwise")
    mytree1 <- untangled.object.temp[[1]]
    mytree2 <- untangled.object.temp[[2]]
  }
  # Prepare trees and append metadata
  if (left_ladder == F){
    mytree1.gg <- ggtree(mytree1, size=0.4, ladderize = F)
  } else {
    mytree1.gg <- ggtree(mytree1, size=0.4, ladderize = T, right=T)  
  }
  mytree1.gg <- mytree1.gg %<+% mymetadata +
    geom_tippoint(aes_string(color=colvar1.name), size=pointsize, alpha=point_alpha) +
    theme.text.size  
  
  if (right_ladder == F){
    mytree2.gg <- ggtree(mytree2, size=0.4, ladderize = F)
  } else {
    mytree2.gg <- ggtree(mytree2, size=0.4, ladderize = T, right=T)
  }
  ...
  mytree1.d <- mytree1.gg$data
  mytree2.d <- mytree2.gg$data
  ...
  mytrees.pp <- mytree1.gg + 
    ...
    geom_point(data=mytree2.meta, aes_string("x", "y", color=colvar1.name), size=pointsize, alpha=point_alpha) +
    ggnewscale::new_scale_fill() 
  ...
  # plot combined tree with intersection lines
  mytrees_tangle.lines.ggtree <- mytrees.pp + geom_line(aes_string("x", "y", group="label",color=col.variable.name), data=mytrees.dd,  alpha=line_alpha, size=0.25)
  ...
  mytrees_tangle.lines.ggtree <- mytrees_tangle.lines.ggtree + geom_text(data=mytrees_tanglegram.labels, aes(y=label.y, x=label.x, label=label.text)) +
    theme(legend.key.size = unit(0.55,"line"),legend.position='right') +
    scale_color_manual(breaks=colorscheme.temp$variable, values=colorscheme.temp$color, name="Sublineage",na.value = NA, guide = guide_legend(order=1))
...
testing
```{r, eval=F}
# test.tree1 <- ggtree(WGS.ML.tree.ult, size=0.4, right=T) %<+% data.frame(Sample_Name=TPAE.pinecone.10$Taxa, TPAE.pinecone=TPAE.pinecone.10$TPA.TPE.scheme, stringsAsFactors=F) 
# 
# test.tree1$data

#cophylo.object.temp <- phytools::cophylo(WGS.ML.tree.ult, revised_candidate.amplicon.SNP.seqs.alignment.tree, rotate.multi=T)

# cophylo.object.temp <- phytools::untangle(c(WGS.ML.tree.ult, revised_candidate.amplicon.SNP.seqs.alignment.tree), method = "stepwise")
```
...
p.WGS_dataset_vs_56amplicon_finalsites_tanglegram <- plot_tanglegram(WGS.ML.tree.ult, revised_candidate.amplicon.SNP.seqs.alignment.tree, data.frame(Sample_Name=TPAE.pinecone.10$Taxa, TPAE.pinecone=TPAE.pinecone.10$TPA.TPE.scheme, stringsAsFactors=F), colorscheme = TPA.TPE.scheme.cols, scale_difference = 1750, centre_separation = 250, untangle=T)
...
p.WGS_dataset_vs_56amplicon_finalsites_tanglegram__TPA.only <- plot_tanglegram(keep.tip(WGS.ML.tree.ult, filter(TPA.meta1, Cleaned_fastq_id %in% WGS.ML.tree.ult$tip.label, Species=="TPA") %>% pull(Cleaned_fastq_id) ), 
                ...
                data.frame(Sample_Name=TPAE.pinecone.10$Taxa, TPAE.pinecone=TPAE.pinecone.10$TPA.TPE.scheme, stringsAsFactors=F), TPA.TPE.scheme.cols, scale_difference = 1000, centre_separation = 10, untangle=T) +
  ggtitle("TPA only")
...
p.WGS_dataset_vs_56amplicon_finalsites_tanglegram__TPE.only <- plot_tanglegram(keep.tip(WGS.ML.tree.ult, filter(TPA.meta1, Cleaned_fastq_id %in% WGS.ML.tree.ult$tip.label, Species=="TPE") %>% pull(Cleaned_fastq_id) ), 
                ...
                data.frame(Sample_Name=TPAE.pinecone.10$Taxa, TPAE.pinecone=TPAE.pinecone.10$TPA.TPE.scheme, stringsAsFactors=F), TPA.TPE.scheme.cols, scale_difference = 250, centre_separation = 5, pointsize=2, untangle=T) +
  ggtitle("TPE only")
...
plot_tanglegram(keep.tip(WGS.ML.tree.ult, filter(TPA.meta1, Cleaned_fastq_id %in% WGS.ML.tree.ult$tip.label, Species=="TPA", TPA_Lineage=="SS14") %>% pull(Cleaned_fastq_id) ), 
                ...
                data.frame(Sample_Name=TPAE.pinecone.10$Taxa, TPAE.pinecone=TPAE.pinecone.10$TPA.TPE.scheme, stringsAsFactors=F), TPA.TPE.scheme.cols, scale_difference = 1000, centre_separation = 5, untangle=T) +
...
plot_tanglegram(keep.tip(WGS.ML.tree.ult, filter(TPA.meta1, Cleaned_fastq_id %in% WGS.ML.tree.ult$tip.label, Species=="TPA", TPA_Lineage=="Nichols") %>% pull(Cleaned_fastq_id) ), 
                ...
                data.frame(Sample_Name=TPAE.pinecone.10$Taxa, TPAE.pinecone=TPAE.pinecone.10$TPA.TPE.scheme, stringsAsFactors=F), TPA.TPE.scheme.cols, scale_difference = 500, centre_separation = 10, pointsize=2, untangle=T) +
...
PhyloSeq_Amplicon_tree.fixed.tre.noSing.pairsnp__dist.melt <- data.frame(candidate.amplicon.SNP.seqs.alignment.distances, check.names = F) %>% 
  rownames_to_column("Taxa1") %>%
  pivot_longer(-Taxa1, names_to = "Taxa2", values_to = "Distance.Phylo")


# plot distribution of distances
PhyloSeq_Amplicon_tree.fixed.tre.noSing.pairsnp__dist.melt %>%
  ggplot(aes(Distance.Phylo)) + geom_histogram() + theme_bw()
```
\
\
If we use pairsnp to do this, there is a problem, because the WGS data we are inferring from contains low coverage sites marked as 'N' which pairsnp does not count - they are considered 'identical'. This can be a problem for network building. On the other hand, if we include every 'N' sites, it would become much more messy than the reality (in which we would have high depth ONT SNP calls. 

For the purposes of this in silico demonstration, we will use ancestrally reconstructed SNPs at the internal nodes (i.e. via pyjar) to calculate the SNP distances between samples. 

```{r}
# import pyjar tree of amplicons
revised_candidate.amplicon.SNP.seqs.alignment.pyjar.tree <- midpoint.root(read.tree(revised_candidate.amplicon.SNP.seqs.alignment.pyjar.tree.filename))

# extract pairwise cophenetic distance matrix from tree
PhyloSeq_Amplicon_tree.fixed.tre.pyjar__dist <- ape::cophenetic.phylo(revised_candidate.amplicon.SNP.seqs.alignment.pyjar.tree)

PhyloSeq_Amplicon_tree.fixed.tre.pyjar__dist.melt <- data.frame(PhyloSeq_Amplicon_tree.fixed.tre.pyjar__dist, check.names = F) %>% 
  rownames_to_column("Taxa1") %>%
  pivot_longer(-Taxa1, names_to = "Taxa2", values_to = "Distance.Phylo")
  
PhyloSeq_Amplicon_tree.fixed.tre.pyjar__dist.melt
...
```{r}
PhyloSeq_Amplicon_tree.fixed.tre.noSing__dist.melt <- PhyloSeq_Amplicon_tree.fixed.tre.pyjar__dist.melt
...
# Remove singleton sublineages from this analysis
```
\
...
Amplicon_dataset.0d.network.components <- data.frame(Taxa1=network.vertex.names(Amplicon_dataset.0d.network), vertex.no=as.vector(V(Amplicon_dataset.0d.network.ig)), cluster.temp=igraph::components(Amplicon_dataset.0d.network.ig)$membership)

# Relabel clusters based on size
Amplicon_dataset.0d.network.components <- Amplicon_dataset.0d.network.components %>%
  group_by(cluster.temp) %>%
  summarise(cluster_size = n(), .groups = "drop") %>%
  arrange(desc(cluster_size)) %>%
  mutate(cluster.id = row_number()) %>%
  right_join(Amplicon_dataset.0d.network.components, by="cluster.temp") %>%
  mutate(cluster=cluster.id) %>%
  select(Taxa1, vertex.no, cluster)
 # Amplicon_dataset.0d.network.components
...
Amplicon_dataset.0d.network.gg__distinct.SNPcluster__sumstats <- Amplicon_dataset.0d.network.gg__distinct.SNPclusters %>%
  #filter(Population_sl != "sublin-Singleton") %>% # remove singletons from sublineage list
...
```
\
Explore the mixtures a bit more 
```{r}
p.Amplicon_dataset.0d.network.gg__distinct.SNPcluster__sublineage.composition.bar_mixeds
...
Amplicon_dataset.0d.network.gg__distinct.SNPclusters
Amplicon_dataset.0d.network.gg
...
#ggsave(plot=p.collapsed_WGS.sublineages.tree_with_identical.clusters_w.bar_combi, paste0(Figures_directory,"SuppFig11__Amplicon_Sublin-discrimination_tree.",format(Sys.Date(),"%Y%m%d"),".pdf"), units='mm', width=260, height=200, device='pdf', dpi=400) 
...
UK_Illumina_data.__AmpliSeq_sim.seqs <- UK_Illumina_data.__AmpliSeq_sim.df.spread %>% 
  tidyr::unite("sequence", sep = "", -Indiv, remove = TRUE)
...
p.UK_Illumina_data.__AmpliSeq_sim.pyjar.grapetree <- ggdraw() + draw_image(UK_Illumina_data.__AmpliSeq_sim.pyjar.grapetree.file, interpolate = T)

p.UK_Illumina_WGS_grapetree <- ggdraw() + draw_image(UK_Illumina_WGS_grapetree.file, interpolate = T)
...
TP20230502SA.region.depth.good.samples.tab <- TP20230502SA.region.depth.meta %>% 
  ...
  mutate(poor.performing.samples=ifelse(mean.cov<50, "Poor","ok")) # classify samples as poor/ok based on mean coverage <50x
...
TP20230502SA.region.depth.good.samples.list2 <- TP20230502SA.region.depth.good.samples.tab  %>%
  ...
  pull(barcode) # make a list of 'good samples' with mean cov >=50x


####################
# Now look at the amplicons 
# First remove the poorly performing samples (so they don't excessively bias the analysis)
TP20230502SA.region.depth.consistent.amplicons.tab <- TP20230502SA.region.depth.meta %>% 
  filter(barcode %in% TP20230502SA.region.depth.good.samples.list2) %>% # filter genomes to have mean cov >50x
  group_by(Region) %>%
  #mutate(quant.10pc=quantile(Mean_Cov, 0.10)) %>% relocate(quant.10pc) # checking
  # so the above effectively says "what is the mean coverage for the 10% quantile (i.e. top of the bottom 10%) in each region/amplicon?"
###### Calculate metrics for each amplicon - what is the mean coverage for the 10% quantile in each amplicon/region?
  ...
  # filters out amplicons where the the 10th percentile for mean amplicon coverage is <25x
  # specifically filters amplicons
  filter(quant.10pc>25)
TP20230502SA.region.depth.consistent.amplicons.tab


# So there are two filters:
# - Mean coverage > 50x for total sample (filters samples)
# Amongst the good samples:
# - >90% of samples must achieve >25x mean coverage (filters amplicons)
...
p.SA.meancov_vs_Ct.facet.scatter <- TP20230502SA.region.depth.meta %>%
  ...
  mutate(Region.num = as.numeric(gsub("region\\_","",Region.alt))) %>%
  arrange(Region.num) %>%
  mutate(Region.alt = factor(Region.alt, levels=unique(Region.alt))) %>% # to order facets numerically
  ...
  ggplot(aes(Tp47_qPCR_Ct, Mean_Cov, color=good.amplicon)) +
    ...
    geom_hline(yintercept=50, color='blue', linewidth=0.5, alpha=0.8) +
...
TP20230502SA.region.depth__mean.sample.cov <- TP20230502SA.region.depth.meta %>% 
  group_by(barcode) %>%
  summarise(mean.cov=mean(Mean_Cov)) %>%
  mutate(MinION_Sequencing_Run = gsub("\\_barcode.+$","", barcode)) %>%
  mutate(SeqRun = recode(MinION_Sequencing_Run, 'SA1_20230426' = 'Run1', 'SA2_20230428' = 'Run2', 'SA3_20230502' = 'Run3'))
...
p.SA.meancov_vs_Ct__per_sample.facet.scatter <- TP20230502SA.region.depth.meta %>%
  ...
  #stat_summary(fun = mean, geom = "crossbar", width = 1.2, color = "cyan", fatten = 0.75) +
  geom_crossbar(data = TP20230502SA.region.depth__mean.sample.cov, aes(x = barcode, y = mean.cov, ymin = mean.cov, ymax = mean.cov), width = 1.2, color = "cyan", fatten = 0.75, inherit.aes = F) +
  scale_y_log10() +
  theme_bw() +
  facet_grid(.~SeqRun, scale = "free_x", space = "free_x") +
  x.theme.axis.rotate.angle +
  theme.text.size + 
...
p.SA.qPCR_values.scatter <- TP20230502SA.region.depth.meta %>%
  distinct(barcode, Tp47_qPCR_Ct, .keep_all = T) %>%
  arrange(Tp47_qPCR_Ct) %>%
  mutate(barcode=factor(barcode, levels=unique(barcode))) %>%
  mutate(SeqRun = recode(MinION_Sequencing_Run, 'SA1_20230426' = 'Run1', 'SA2_20230428' = 'Run2', 'SA3_20230502' = 'Run3')) %>%
...
p.SA.meancov_vs_Sample_vs_Ct__per_sample.combi <- plot_grid(p.SA.qPCR_values.scatter + x.theme.strip.labs + theme(plot.margin = unit(c(0,0,0,0), units = "lines")), p.SA.meancov_vs_Ct__per_sample.facet.scatter + theme(axis.text.x = element_blank()) + theme(strip.text.x = element_blank(), strip.background = element_blank(), plot.margin = unit(c(0,0,0,0), units = "lines") ), ncol=1, rel_heights = c(1,4), axis="lrtb", align="v", scale=0.95)
...
p.Zim.meancov_vs_Ct__per_sample.scatter <- collated.cov.zim %>%
  ...
  theme.text.size + 
  labs(y="Mean Coverage (X) per amplicon", x="Sample") +
  stat_summary(fun = median, geom = "crossbar", width = 1.2, color = "cyan", fatten = 0.75) +
...
#ggsave(plot=p.Zim.meancov_vs_Sample_vs_Ct__per_sample.combi, paste0(Figures_directory,"SuppFig16__Zim-PCR-sample-cov.",format(Sys.Date(),"%Y%m%d"),".pdf"), units='mm', width=120, height=100, device='pdf', dpi=400)
```
\
\
\
...
#ggsave(plot=p.simulated_mutations_in_amplicons, paste0(Figures_directory,"SuppFig17__Simulate-mutational-freqs.",format(Sys.Date(),"%Y%m%d"),".pdf"), units='mm', width=180, height=180, device='pdf', dpi=400)
```
...
########## 
# Revisions - April-August 2025
## New analyses added (others integrated earlier)

\
\
### Rerun SA data from R9.4.1 flowcells using updated ONTAP pipeline 
\
The South African ONT data was run using R9.4.1 chemistry, and analysed using an in house pipeline. This was subsequently replaced with the ONTAP pipeline (https://github.com/sanger-pathogens/ONTAP) for field work in Zimbabwe. A number of refinements and additional QC steps were introduced for ONTAP, and these should be used to confirm the discrepant SNPs identified between Illlumina and ONT in the South African data are resolved. 

merged vcf file: R9_SA1+2+3_allsites.merge.vcf 

```{r}
# Import variant calls from new ONTAP pipeline
# SA_ONT_R9_variants_ONTAP_repeat_calls.file <- paste0(Data_input_directory, "ONTAP_R9_SA1+2+3_allsites.merge.vcf") 

# Read in merged vcf from ONTAP pipeline analysis of ONT data
SA_ONT_R9_variants_ONTAP_repeat_calls <- read_and_process_vcf(SA_ONT_R9_variants_ONTAP_repeat_calls.file)
SA_ONT_R9_variants_ONTAP_repeat_calls <- SA_ONT_R9_variants_ONTAP_repeat_calls %>%
  mutate(Indiv = paste("ONTAP_",Indiv)) %>%
  filter(!grepl("Unclassified_reads",Indiv))
#SA_ONT_R9_variants_ONTAP_repeat_calls

# Filter and process in line with earlier comparison
SA_ONT_R9_variants_ONTAP_repeat_calls.filt <- SA_ONT_R9_variants_ONTAP_repeat_calls %>% 
  mutate(readsetID = gsub("ONTAP.+SA","SA",Indiv)) %>% 
  mutate(readsetID = gsub("SA3_20230503","SA3_20230502",readsetID)) %>% #fix slight mislabelling issue
  #pull(readsetID) %>% tail()
  left_join(SA.ampliseq.comparison.meta, by=c("readsetID"="readsetID")) %>%
  mutate(SeqMethod = gsub("ONT_PhyloSeq","ONTAP_PhyloSeq", SeqMethod)) %>%
  filter(!is.na(`Sample Name`)) %>%
  mutate(gt_GT_simplify= ifelse(gt_GT=="0/1","0",
                                ifelse(gt_GT=="1","1", 
                                       ifelse(gt_GT=="1/1","1",
...
# Now combine the data from the 3 different datasets using harmonised columns
df.SA_ONT_R9_vs_ONTAP_vs_Illumina_meta_combined <- rbind( select(SA.Ampliseq_vs_IlluminaWGS.df.meta.sel, `Sample Name`, Indiv, SeqMethod, POS, REF, gt_GT_alleles, gt_GT, gt_GT_simplify), select(SA_ONT_R9_variants_ONTAP_repeat_calls.filt, `Sample Name`, Indiv, SeqMethod, POS, REF, gt_GT_alleles, gt_GT, gt_GT_simplify) )

df.SA_ONT_R9_vs_ONTAP_vs_Illumina_meta_combined  
...
\
```{r}
df.SA_ONT_R9_vs_ONTAP_vs_Illumina_meta_combined.var_comparison <- df.SA_ONT_R9_vs_ONTAP_vs_Illumina_meta_combined %>% 
  group_by(`Sample Name`, POS) %>%
  mutate(res_per_sample.var=n()) %>%
  ungroup() %>%
  filter(POS!=1072103) %>%
  group_by(`Sample Name`, POS) %>%
  mutate(nvars.persite.persample=length(unique(gt_GT_simplify)), concordant.in.triplet=ifelse(nvars.persite.persample==1,'Concordant', 'Discrepant')) %>%
  ungroup() %>%
  arrange(POS, SeqMethod)

df.SA_ONT_R9_vs_ONTAP_vs_Illumina_meta_combined.var_comparison %>%
  filter(nvars.persite.persample !=1) %>%
  # select(SeqMethod, `Sample Name`, POS, REF, res_per_sample.var, nvars.persite.persample, concordant.in.triplet, gt_GT_simplify) %>%
  select(SeqMethod, Indiv, `Sample Name`, POS, REF, res_per_sample.var, nvars.persite.persample, concordant.in.triplet, gt_GT_simplify) #%>%
  # pivot_wider(names_from=SeqMethod, values_from=gt_GT_simplify)
```
\
First check variants that were previously a problem:
7002
```{r}
df.SA_ONT_R9_vs_ONTAP_vs_Illumina_meta_combined.var_comparison %>%
  filter(nvars.persite.persample !=1) %>%
...
```
ONTAP gvcfs:
`ONTAP_ TP_Discriminatory_SA2_20230428_barcode06` - Identified as 'T' - INDEL missed.
The ONTAP pipeline is still not handling INDELS correctly, and this has been missed. 
...
```
ONTAP gvcfs:
ONTAP_ TP_Discriminatory_SA1_20230426_barcode04 - no variant call (data missing and below threshold)
`NC_021508.1     916085  .       A       <NON_REF>       0       .       END=916107      GT:GQ:MIN_DP:PL 0:1:0:0,0,0`
ONTAP_ TP_Discriminatory_SA1_20230426_barcode07 - only one read available after filtering
`NC_021508.1     916096  .       C       <NON_REF>       0       .       END=916096      GT:GQ:MIN_DP:PL .:0:1:29,3,0`

\ 
So the filter still does not see very low coverage SNPs (as we would expect - we are being robust). Does it process them correctly when converting to a MSA? Code checked - yes it does. 
...
Want to compare MLST calls with phylogeny (and phylogenetic sublineage calls) in response to reviewer comments.
\
```{r}
...
#ggsave(plot=p.Ampliseq_WGS_tree_subset.withMLST.gg, paste0(Figures_directory,"SuppFig-__WGS-phylo_MLST.",format(Sys.Date(),"%Y%m%d"),".pdf"), units='mm', width=220, height=200, device='pdf', dpi=800) 
```
\
\
\
...
`snippy-multi ../seqnames_reads.tsv --ref ../NC_002946__FA1090.gb --cpus 8 > snippy-multi-submitfile.txt`
Multiple sequence alignment built using snippy-core, then fix sites using inbuilt snippy tools.
Create VCF using snp-sites `snp-sites -v -o clean.SNPs.vcf clean.SNPs.aln`
\
```{r}
Taouk_gono_vcf.file <- paste0(Gono_data,"clean.SNPs.vcf.gz")
```
\
\
...
`Rscript Process_multi-VCF_to_CSV.R clean.SNPs.vcf.gz`
...
Taouk_gono.processed.vcf.melt <- Taouk_gono.processed.vcf %>%
  pivot_longer(-Indiv, names_to="POS", values_to="Allele")
  
Taouk_gono.processed.vcf.melt
```
\
...
```{r}
Taouk_gono.metadata <- read_excel(Taouk_gono.metadata.file, sheet='cleaned_up')
...
Taouk_gono.metadata %>%
  ...
  theme_bw() + theme.text.size +
  scale_y_log10() + scale_x_continuous(breaks=seq(0,800,50))

# Let's pull out the most common clusters (i.e. those with 10 or more examples)
Taouk_gono.metadata_min10 <- Taouk_gono.metadata %>%
  group_by(`Cluster ID`) %>%
  summarise(count=n()) %>%
  filter(count>=10)
Taouk_gono.metadata_min10

Taouk_gono.metadata_min10.list <- Taouk_gono.metadata_min10 %>%
  pull(`Cluster ID`)
Taouk_gono.metadata_min10.list

Taouk_gono.metadata <- Taouk_gono.metadata %>%
  mutate(cluster_limited = ifelse(`Cluster ID` %in% Taouk_gono.metadata_min10.list, `Cluster ID`, "Other"))
Taouk_gono.metadata

#write.csv(select(Taouk_gono.metadata, Accession, cluster_limited), file="Gono_data/Taouk_clusters_min10.csv", row.names = F)
```

Define a color scheme for the Taouk sublineages/clusters
```{r}
Taouk_clusters.cols <- Taouk_gono.metadata %>% 
  mutate(cluster = as.numeric(cluster_limited)) %>%
  distinct(cluster) %>%
  arrange(cluster) %>%
  filter(!is.na(cluster))

# Taouk_clusters.cols$cluster.cols <- createPalette(length(Taouk_clusters.cols$cluster),  c("#ff0000", "#00ff00", "#0000ff"))

Taouk_clusters.cols$cluster.cols <- c("#F80D0D", "#2AFE0D", "#2A26FA", "#F7CBCD", "#FF00E2", "#0DCEFD", "#F3E50D", "#227622", "#E37A00", "#6C2A84", "#CF166C", "#0DFEDA", "#75351C", "#CA00FF", "#166671", "#F885D0", "#9399FB", "#ABF175", "#A79342", "#C0ECC5", "#CBD6FF", "#FB9197", "#DF82F8", "#F5564D", "#47492A", "#26FC9B", "#FBB900", "#93607E", "#940D5C", "#7C1CC1", "#7CDBDF", "#165396", "#FD0DB4", "#951C22", "#F7BBFF", "#AA0099", "#F9B27E", "#F80D6C", "#98A39F", "#00A600", "#DFE889", "#16A283", "#1685FF", "#766E9A", "#CE16D7", "#69CE85", "#7D9C16", "#16A7F7", "#9B7A6A", "#FDDEAF", "#5D9BC4", "#839B66", "#FA71A8", "#9BFB16", "#E290AD", "#B76038", "#CBEE16", "#FF73ED", "#3247BA", "#FFDB73", "#9F7DCD", "#B666B5", "#9535F8", "#C91691", "#C9ACCA", "#F7697C", "#166040", "#9400A9", "#FE0092", "#8575FD", "#00F2FF", "#A3703B", "#5CED5D", "#862271", "#DC9B8F", "#C6E6EB", "#C2900D", "#BC76FF", "#4D4953")
...
Also need to run FST classification. This has also been rewritten as a command line script
 ...
 `Rscript scripts/Calculate_FST_from_populations.R clean.SNPs.vcf_processed_spread.tsv Gono_data/Taouk_clusters_min10.csv 0.99`
...
Taouk_gono.FST.sig <- Taouk_gono.FST %>% filter(sig=="sig")
Taouk_gono.FST.sig

# How many unique sites have sufficient FST
Taouk_gono.FST.sig %>% 
  distinct(POS)
```
\
...
\
Some sites may support multiple sublineages (at the 0.95 FST cuttoff used). Check if it makes sense to filter them out
```{r}
Taouk_gono.FST.sig %>%
  group_by(POS) %>%
  summarise(pop.count=n()) %>%
  arrange(desc(pop.count))
  

Taouk_gono.FST.sig_single.sublin.sites.list <- Taouk_gono.FST.sig %>%
  group_by(POS) %>%
  summarise(pop.count=n()) %>% 
  filter(pop.count==1) %>%
  pull(POS)

Taouk_gono.FST.sig_single.sublin.sites.list


Taouk_gono.FST.sig_single.sublin.sites <- Taouk_gono.FST.sig %>%
  filter(POS %in% Taouk_gono.FST.sig_single.sublin.sites.list)

Taouk_gono.FST.sig_single.sublin.sites
```

Examine sites per sublineage
```{r, fig.width=8, fig.height=8}
p.Taouk_gono.FST.sig.sublins_point.plot <- Taouk_gono.FST.sig %>%
  ...
  theme_bw() + theme.text.size + theme(legend.key.size = unit(0.55,"line"),legend.position='right') +
  x.theme.axis.rotate +
  scale_color_manual(breaks=Taouk_clusters.cols$cluster, values=Taouk_clusters.cols$cluster.cols) +
  labs(y="Discriminatory Site Count", x="Genomic Sublineage", fill="Genomic\nSublineage") +
  labs(y=expression(F[ST]), x="Genome Position", color=expression(paste("Significant ", F[ST]))) +
  coord_cartesian(y=c(0.75,1) ) +
  labs(title=paste0(nrow(Taouk_gono.FST.sig %>% distinct(POS)), " discriminating alleles across all 77 defined sublineages")) +
  facet_wrap(vars(population)) + theme(legend.position = 'none')
p.Taouk_gono.FST.sig.sublins_point.plot

p.Taouk_gono.FST.sig.sublins_point_singlesub.plot <- Taouk_gono.FST.sig_single.sublin.sites %>%
  #mutate(population=factor(population)) %>%
  arrange(POS) %>% distinct(POS, population, .keep_all = T) %>%
  ggplot(aes(x=POS, y=FST, color=population)) +
  geom_point(alpha=0.5, size=1) + 
  ...
  labs(title=paste0(nrow(Taouk_gono.FST.sig %>% distinct(POS)), " discriminating alleles across all 77 defined sublineages")) +
  facet_wrap(vars(population)) + theme(legend.position = 'none')
p.Taouk_gono.FST.sig.sublins_point_singlesub.plot



# bar plots
p.Taouk_gono.FST.sig.raw_sublins_support_bar <- Taouk_gono.FST.sig %>% 
  distinct(population, POS) %>%
  dplyr::group_by(population) %>%
  dplyr::summarise(count=n()) %>%
  dplyr::mutate(sublineage=(gsub("sublin-","",population))) %>%
  #dplyr::arrange(sublineage) %>%
  dplyr::arrange(count) %>%
  dplyr::mutate(population=factor(population,levels=unique(population))) %>%
  ungroup() %>%
  ggplot(aes(x=population, y=count, fill=population)) +
  geom_bar(stat='identity', width=0.65) + 
  theme_bw() + theme.text.size + theme(legend.key.size = unit(0.55,"line"),legend.position='top') + 
  scale_y_continuous(breaks=pretty) +
  scale_fill_manual(breaks=Taouk_clusters.cols$cluster, values=Taouk_clusters.cols$cluster.cols) +
  labs(y="Discriminatory Site Count", x="Genomic Sublineage", fill="Genomic\nSublineage") +
  labs(y="Discriminatory Site Count", x="Genomic Sublineage", fill="Genomic\nSublineage") +
  x.theme.axis.rotate.angle +
  theme(strip.text.x = element_blank()) +
  geom_text(aes(x=population, y=count+12, label=count),size=text.size.within-0.5, angle = 90) +
  theme(legend.position = 'none') +
  NULL
p.Taouk_gono.FST.sig.raw_sublins_support_bar

p.Taouk_gono.FST.sig.raw_sublins_singlesub_support_bar <- Taouk_gono.FST.sig_single.sublin.sites %>% 
  distinct(population, POS) %>%
  dplyr::group_by(population) %>%
  dplyr::summarise(count=n()) %>%
  dplyr::mutate(sublineage=(gsub("sublin-","",population))) %>%
  ...
  theme(legend.position = 'none') +
  NULL
p.Taouk_gono.FST.sig.raw_sublins_singlesub_support_bar
```
As previously, some sublineages in the Taouk Gonorrhoeae dataset have limited SNP support, whilst others are extremely well supported. Need to apply the selection algorithm, but first check for optimal positional tuning.

```{r}
# Update positional matrix function to take any dataset (not specifically the Tp one)
######################################################################
make_genome_site_positional_matrix_2 <- function(region.size, sig.sites.df) {
  sig.sites.list <- sort(unique(sig.sites.df$POS))
  FST.unique.sites.combo <- data.frame(t(combn(sig.sites.list,2,simplify=T)),stringsAsFactors = F)
  colnames(FST.unique.sites.combo) <- c("site1","site2")
...
```
\
Takes a while with the bigger gonorrhoea dataset
```{r, eval=FALSE}
#make_genome_site_positional_matrix_2(100, Taouk_gono.FST.sig)

# Run analysis in a loop to assess impact of changing cluster size
genomic_positional_distance_clustering_collated_gono <- setNames(data.frame(matrix(ncol = 4, nrow = 0)), c("region.cluster", "SNPs.per.cluster", "region.distance", "linkage.dist"))

for (current.size in seq(50,1000, 50)){
  # Generate network
  FST.sublin.combined.sig.unique.sites.combo.near_gono__current <- make_genome_site_positional_matrix_2(current.size, Taouk_gono.FST.sig)
  genomic.sites.network_current_gono <- make_positional_network(FST.sublin.combined.sig.unique.sites.combo.near_gono__current)
  # Get cluster info and link
  genomic.sites.network_current_gg_gono <- prepare_networkgg_for_plotting(genomic.sites.network_current_gono) 
  # extract key stats on network components
  genomic.sites.network_current_gg_gono <- genomic.sites.network_current_gg_gono %>%
    group_by(region.cluster) %>%
    mutate(SNPs.per.cluster=length(unique(vertex.names))) %>%
    mutate(vertex.pos=as.numeric(gsub("s","",vertex.names))) %>%
    mutate(region.start=min(vertex.pos), region.end=max(vertex.pos), region.distance=region.end-region.start) %>%
    ungroup() 
    # Create summary df that contains the key info we need
    summarise.sites.network.info_gono <- genomic.sites.network_current_gg_gono %>% 
      mutate(linkage.dist=current.size) %>%
      select(region.cluster, SNPs.per.cluster, region.distance, linkage.dist) %>%
      distinct()
   
  genomic_positional_distance_clustering_collated_gono <- rbind(summarise.sites.network.info_gono, genomic_positional_distance_clustering_collated_gono)
}
  
genomic_positional_distance_clustering_collated_gono

#write.csv(genomic_positional_distance_clustering_collated_gono, file=paste0("Gono_data/", "genomic_positional_distance_clustering_collated_gono.csv"), row.names = F)
```
\
\
read back in
```{r, fig.height=6, fig.width=6}
genomic_positional_distance_clustering_collated_gono <- read.csv(paste0("Gono_data/", "genomic_positional_distance_clustering_collated_gono.csv.gz"))
```
\
Now make some plots
```{r, fig.height=8, fig.width=8} 
p.genomic_positional_distance_clustering_Clustercount.gono <- genomic_positional_distance_clustering_collated_gono %>%
  group_by(linkage.dist) %>%
  summarise(cluster.count = n()) %>%
  ggplot(aes(x=linkage.dist, y=cluster.count)) +
  geom_point(alpha=0.5, size=1) + 
  theme_bw() + theme.text.size +
  scale_x_continuous(breaks=seq(0,max(genomic_positional_distance_clustering_collated_gono$linkage.dist), 100)) +
  coord_cartesian(ylim=c(0,1200)) +
  scale_y_continuous(breaks=pretty) +
  x.theme.axis.rotate.angle +
  labs(x="Cluster linkage size (bp)", y="Cluster count")

p.genomic_positional_distance_clustering_SNPcount.gono <- genomic_positional_distance_clustering_collated_gono %>%
  ggplot(aes(x=linkage.dist, y=SNPs.per.cluster)) +
  rasterise(geom_quasirandom(size=0.5, alpha=0.5), dpi=400) + 
  theme_bw() + theme.text.size +
  scale_x_continuous(breaks=seq(0,max(genomic_positional_distance_clustering_collated_gono$linkage.dist), 100)) +
...
p.genomic_positional_distance_clustering_regiondist.gono <- genomic_positional_distance_clustering_collated_gono %>%
  ggplot(aes(x=linkage.dist, y=region.distance)) +
  rasterise(geom_quasirandom(size=0.5, alpha=0.5), dpi=400) + 
  theme_bw() + theme.text.size +
  scale_x_continuous(breaks=seq(0,max(genomic_positional_distance_clustering_collated_gono$linkage.dist), 100)) +
  scale_y_continuous(breaks=pretty) +
  scale_y_log10() + coord_cartesian(ylim=c(1,max(genomic_positional_distance_clustering_collated_gono$region.distance))) +
  x.theme.axis.rotate.angle +
  labs(x="Cluster linkage size (bp)", y="Length of region\nspanning SNPs (bp)")

# Combination plot
p.genomic_positional_distance_clustering__combi.gono <- plot_grid(p.genomic_positional_distance_clustering_Clustercount.gono + x.theme.strip, p.genomic_positional_distance_clustering_SNPcount.gono + x.theme.strip, p.genomic_positional_distance_clustering_regiondist.gono, ncol=1, align="v", rel_heights = c(2,3,4), label_size = 11, labels = "AUTO")

p.genomic_positional_distance_clustering__combi.gono


#ggsave(plot=p.genomic_positional_distance_clustering__combi.gono, paste0(Figures_directory,"SuppFig13__Linkage-distance-impact.gono",format(Sys.Date(),"%Y%m%d"),".pdf"), units='mm', width=180, height=180, device='pdf', dpi=400) 

```
\
\
Now create a set of regions for clustering (to optimise regions)
\
The above plots show there are far more sites available within a small amplicon than were present in the Tp dataset. However, for the purposes of direct comparison, we will use the same thresholds as for Tp. 

```{r, fig.width=12, fig.height=12}
# Make positional matrix using 300 bp linkage setting (as for Tp)
Taouk_gono__FST.sublin.combined.sig.unique.sites.combo.near <- make_genome_site_positional_matrix_2(300, Taouk_gono.FST.sig_single.sublin.sites) #Taouk_gono.FST.sig)

###########
genomic.sites.network_gono <- make_positional_network(Taouk_gono__FST.sublin.combined.sig.unique.sites.combo.near)

genomic.sites.network.gg_gono <- prepare_networkgg_for_plotting(genomic.sites.network_gono)

# Add metadata for which sublineage is supported by each site
genomic.sites.network.gg_gono <- Taouk_gono.FST.sig %>%
  mutate(vertex.names = paste0("s",POS)) %>%
  right_join(genomic.sites.network.gg_gono, by="vertex.names") %>%
  select(-POS) %>%
  distinct()

# Visualise network
p.genomic.sites.network.gg_gono <- ggplot(genomic.sites.network.gg_gono, aes(x = x, y = y, xend = xend, yend = yend)) + 
  geom_edges(alpha=0.25,curvature = 0, color='grey25') + 
  #geom_nodes(size=2, alpha=0.5, aes(color=region.cluster)) +
  geom_nodes(size=2, alpha=0.5, aes(color=population)) +
  scale_color_manual(breaks=Taouk_clusters.cols$cluster, values=Taouk_clusters.cols$cluster.cols) +
  theme_blank() + theme(legend.position='none') +
  NULL
p.genomic.sites.network.gg_gono
```

\
Extract information about the network:
```{r}
genomic.sites.network.gg_gono %>%
  distinct(region.cluster)
```


\
Lots of big single-lineage network components likely represent recombination (which is not yet filtered - to redo if possible). 
\
Ok, so Gubbins cannot cope with such a large dataset (timed out after 30 days running). Debatable whether recombination needs to be removed if it is discriminatory. We can instead apply filters to exclude regions that are specific to a single region with many SNPs (i.e likely driven by a recombination block).
\
...
\
Link sublineage information back to network components
```{r}
# Create list of sublineages to support (ideally order so those with the lowest native support go first)
Taouk.sublins.list.ordered <- Taouk_gono.FST.sig %>% 
  distinct(population, POS) %>%
  dplyr::group_by(population) %>%
  dplyr::summarise(count=n()) %>%
  dplyr::arrange(count) %>% pull(population)


Taouk_gono.FST.sig_single.sublin.sites_region.linkage <- Taouk_gono.FST.sig_single.sublin.sites %>%
  arrange(match(population, Taouk.sublins.list.ordered)) %>%
  mutate(vertex.names=paste0("s",POS)) %>%
  left_join(genomic.sites.network.gg_gono, by=c("vertex.names","population","FST")) %>%
  select(POS, region.cluster, population, vertex.names) %>%
  distinct() %>%
  mutate(region.cluster = ifelse(is.na(region.cluster), vertex.names, region.cluster)) # for singleton sites that don't form part of a component cluster, add a vertex name
Taouk_gono.FST.sig_single.sublin.sites_region.linkage
```
\
Get stats about unfiltered network
```{r}
Taouk_gono.FST.sig_single.sublin.sites_region.linkage %>%
  filter(grepl("s",region.cluster)) %>% distinct()

# List singleton-SNP phy-cons
Taouk_gono.FST.sig_single.sublin.sites_region.linkage %>%
  filter(grepl("s",region.cluster)) %>% distinct(region.cluster)

# List multi-SNP phy-cons
Taouk_gono.FST.sig_single.sublin.sites_region.linkage %>%
  filter(!grepl("s",region.cluster)) %>% distinct(region.cluster)
...
Need to remove very large network components
```{r}
# Examine lengths and set boundaries for amplicon design
gono.mp.max.length <- 700
Taouk_gono.FST.sig_single.sublin.sites_region.phycon.boundaries <- Taouk_gono.FST.sig_single.sublin.sites_region.linkage %>%
  dplyr::group_by(region.cluster) %>%
  dplyr::summarise(start=min(POS), end=max(POS), length=(end-start), mp.extra.to.add=ceiling((gono.mp.max.length-length)/2), 
            start.extra=start-mp.extra.to.add, end.extra=end+mp.extra.to.add, length.extra=(end.extra-start.extra), length.adjust=ifelse(length==0,1,length)) %>%
  dplyr::arrange(start.extra)
Taouk_gono.FST.sig_single.sublin.sites_region.phycon.boundaries

# Extract list of regions that are below the maximum length requirement
Taouk_gono.FST.sig_single.sublin.sites_region.phycon.boundaries_goodsize <- Taouk_gono.FST.sig_single.sublin.sites_region.phycon.boundaries %>%
  filter(length<=gono.mp.max.length + 25) %>%
  pull(region.cluster)
Taouk_gono.FST.sig_single.sublin.sites_region.phycon.boundaries_goodsize

Taouk_gono.FST.sig_single.sublin.sites_region.linkage_goodsize <- Taouk_gono.FST.sig_single.sublin.sites_region.linkage %>% 
  filter(region.cluster %in% Taouk_gono.FST.sig_single.sublin.sites_region.phycon.boundaries_goodsize)
Taouk_gono.FST.sig_single.sublin.sites_region.linkage_goodsize
```


Check how many different populations/sublineages each network component supports (it was up to 14 for Tp)
```{r}
# How many sublineages per cluster?
Taouk_gono.FST.sig_single.sublin.sites_region.linkage_pops.per.region <- Taouk_gono.FST.sig_single.sublin.sites_region.linkage_goodsize %>% 
  distinct(region.cluster, population) %>%
  group_by(region.cluster) %>%
  summarise(pop.count=n()) %>%
  arrange(desc(pop.count))
Taouk_gono.FST.sig_single.sublin.sites_region.linkage_pops.per.region

max(Taouk_gono.FST.sig_single.sublin.sites_region.linkage_pops.per.region$pop.count)

```
Maximum number of sublineages supported per region cluster (candidate phycon) is 7
\
\
Now apply hierarchical loop to gono dataset
```{r}
###############################################################
# Core Phylo-Seq hierarchical selection loop
######################################################
...
# Create list of sublineages to support (ideally order so those with the lowest native support go first)
Taouk.sublins.list.ordered <- Taouk_gono.FST.sig_single.sublin.sites_region.linkage_goodsize %>% 
  distinct(population, POS) %>%
  dplyr::group_by(population) %>%
  dplyr::summarise(count=n()) %>%
  dplyr::arrange(count) %>% pull(population)
...
hierarchical.FST.sites.to.keep <- NULL
# Create empty list of populations (sublineages) to be added to
Taouk.scheme.list.cumulative.counts <- data.frame(population = Taouk.sublins.list.ordered, sublineage=c(paste0("sublin-",Taouk.sublins.list.ordered)), Cum.count=0) 

# Set minimum number of supporting sites to aim for in each sublineage 
site.support.threshold <- 3


for (current.count in seq(max(Taouk_gono.FST.sig_single.sublin.sites_region.linkage_pops.per.region$pop.count), 1)){
  
  # within loop, create dataset based on how many pops each site is discriminatory for (many = better)
  pops_per_region.current <- Taouk_gono.FST.sig_single.sublin.sites_region.linkage_pops.per.region %>% dplyr::filter(pop.count==current.count)

  # Now filter dataset to only include those haplotypes and get counts per population
  region_and_pop_counts.current <- Taouk_gono.FST.sig_single.sublin.sites_region.linkage_goodsize %>% 
    dplyr::filter(region.cluster %in% pops_per_region.current$region.cluster) %>%
    dplyr::group_by(region.cluster, population) %>%
    dplyr::summarise(regions.per.pop=n()) %>%
    dplyr::ungroup()
  ...
  # Combine with full list of all sites, and add to cumulative supporting site count
  sites.per.region.current.addCum <- left_join(Taouk.scheme.list.cumulative.counts, sites.per.region.current, by='population') %>% 
    replace(is.na(.), 0) %>%
    dplyr::mutate(Cum.count.new=(Cum.count+Count.per.region)) %>%
    # if the region is not adding to the cumulative counts, reduce to 0
    dplyr::mutate(Cum.count.new.only=ifelse(Cum.count.new < site.support.threshold, Cum.count.new, 0),
  ...
  # add cumulative 'fixed' total back into original spreadsheet (outside of loop)
  Taouk.scheme.list.cumulative.counts$Cum.count <- sites.per.region.current.addCum$Cum.count.new
...
hierarchical.FST.sites.to.keep.gono <- unique(hierarchical.FST.sites.to.keep)


Taouk__genomic.sites.retained.FSTpop.dataframe <- Taouk_gono.FST.sig_single.sublin.sites_region.linkage_goodsize %>% 
  dplyr::filter(region.cluster %in% hierarchical.FST.sites.to.keep.gono) %>% 
  dplyr::ungroup()

Taouk__genomic.sites.retained.FSTpop.dataframe 
```
\
How many amplicons?
```{r}
paste0("There are ",length(unique(Taouk__genomic.sites.retained.FSTpop.dataframe$region.cluster))," region clusters (Phy-cons).")

paste0("There are ",length(unique(Taouk__genomic.sites.retained.FSTpop.dataframe$POS))," unique discriminatory SNPs in those Phy-cons.")

# How many are singletons
Taouk__genomic.sites.retained.FSTpop.dataframe %>%
  filter(grepl("s",region.cluster)) %>% distinct(POS)

# How many are not singletons
Taouk__genomic.sites.retained.FSTpop.dataframe %>%
  filter(!grepl("s",region.cluster)) %>% distinct(POS)
```
\
Check phycon boundaries 
```{r}
Taouk_gono.FST.sig_single.sublin.sites_region.phycon.boundaries_retained <- Taouk_gono.FST.sig_single.sublin.sites_region.phycon.boundaries %>%
  filter(region.cluster %in% unique(Taouk__genomic.sites.retained.FSTpop.dataframe$region.cluster))
Taouk_gono.FST.sig_single.sublin.sites_region.phycon.boundaries_retained

# Make into a table suitable for export
Taouk_gono.phycon.boundaries.output.df <- Taouk_gono.FST.sig_single.sublin.sites_region.phycon.boundaries_retained %>%
  mutate(region=region.cluster, refgenome="NC_002946.2", region.start=start, region.end=end, region.length=length, flanking.region.start=start.extra, flanking.region.end=end.extra, flanking.region.length=length.extra) %>%
  select(region, refgenome, region.start, region.end, region.length, flanking.region.start, flanking.region.end, flanking.region.length)

Taouk_gono.phycon.boundaries.output.df

#write.csv(Taouk_gono.phycon.boundaries.output.df, file=paste0("Gono_data/", "SuppTable6_Gono-PhyloPlex_regions.csv"), row.names = F)
```

\
Check per-population support
```{r}
p.Taouk_gono.FST.sig.phyloplex_refined_support_bar <- Taouk__genomic.sites.retained.FSTpop.dataframe %>%
  group_by(population) %>%
  summarise(SNPs.per.pop = n()) %>%
  arrange(SNPs.per.pop) %>%
  dplyr::mutate(population=factor(population,levels=unique(population))) %>%
  ungroup() %>%
  ggplot(aes(x=population, y=SNPs.per.pop, fill=population)) +
  geom_bar(stat='identity', width=0.65) + 
  theme_bw() + theme.text.size + theme(legend.key.size = unit(0.55,"line"),legend.position='top') + 
  scale_y_continuous(breaks=pretty) +
  labs(y="Discriminatory Site Count", x="Genomic Sublineage", fill="Genomic\nSublineage") +
  ...
  scale_fill_manual(breaks=Taouk_clusters.cols$cluster, values=Taouk_clusters.cols$cluster.cols) +
  theme(strip.text.x = element_blank()) +
  geom_text(aes(x=population, y=SNPs.per.pop+12, label=SNPs.per.pop),size=text.size.within-0.5, angle = 90) +
  theme(legend.position = 'none') +
  NULL
p.Taouk_gono.FST.sig.phyloplex_refined_support_bar
```
\
Compare to before
```{r, fig.height=6, fig.width=8}
df.Taouk_gono_pop_support.raw <- Taouk_gono.FST.sig_single.sublin.sites %>% 
  distinct(population, POS) %>%
  dplyr::group_by(population) %>%
  dplyr::summarise(count=n()) %>%
  dplyr::mutate(sublineage=(gsub("sublin-","",population))) %>%
  ...
  dplyr::mutate(population=factor(population,levels=unique(population))) %>%
  ungroup() %>%
  mutate(dataset = "Raw Sites")

df.Taouk_gono_pop_support.optimised <- Taouk__genomic.sites.retained.FSTpop.dataframe %>% 
  distinct(population, POS) %>%
  dplyr::group_by(population) %>%
  dplyr::summarise(count=n()) %>%
  dplyr::mutate(sublineage=(gsub("sublin-","",population))) %>%
  ...
  dplyr::mutate(population=factor(population,levels=unique(population))) %>%
  ungroup() %>%
  mutate(dataset = "Optimised Sites")

p.Taouk_gono_pop_support_bars__before.after <- rbind(df.Taouk_gono_pop_support.raw, df.Taouk_gono_pop_support.optimised) %>%
  mutate(dataset = factor(dataset, levels=c("Raw Sites","Optimised Sites"))) %>%
  ggplot(aes(x=population, y=count, fill=population)) +
  geom_bar(stat='identity', width=0.65) + 
  facet_grid(dataset~.) +
  theme_bw() + theme.text.size + theme(legend.key.size = unit(0.55,"line"),legend.position='top') + 
  ...
  scale_fill_manual(breaks=Taouk_clusters.cols$cluster, values=Taouk_clusters.cols$cluster.cols) +
  theme(strip.text.x = element_blank()) +
  geom_text(aes(x=population, y=count+15, label=count),size=text.size.within-0.5, angle = 90) +
  theme(legend.position = 'none') +
  NULL
p.Taouk_gono_pop_support_bars__before.after
```
\
Get stats about supported Phy-cons
```{r}
rbind(df.Taouk_gono_pop_support.raw, df.Taouk_gono_pop_support.optimised) %>%
  mutate(dataset = factor(dataset, levels=c("Raw Sites","Optimised Sites")))

length(unique(Taouk_gono.FST.sig_single.sublin.sites$population))
```



\
Replot network, highlighting selected regions

Original code for old plot

Try plotting original network with ellipses indicating selected regions
```{r}
unique(Taouk__genomic.sites.retained.FSTpop.dataframe$region.cluster)
```

\
```{r, fig.width=12, fig.height=12}
Taouk_regions.retained <- unique(Taouk__genomic.sites.retained.FSTpop.dataframe$region.cluster)

Taouk_sites.retained <- filter(Taouk__genomic.sites.retained.FSTpop.dataframe, region.cluster %in% Taouk_regions.retained) %>% distinct(POS) %>% arrange(POS) %>% pull(POS)


# rerunning this code has produced an error in ellipse generation. Take ellipse data out of plotting and calculate separately
ellipse_data <- genomic.sites.network.gg_gono %>%
  mutate(region.cluster.filt = ifelse(region.cluster %in% Taouk_regions.retained, region.cluster, NA)) %>% 
  mutate(region.cluster.present=ifelse(region.cluster %in% Taouk_regions.retained,"present","missing")) %>%
  filter(!is.na(region.cluster.filt)) %>%
  group_by(region.cluster.filt) %>%
  mutate(needs_jitter = n() <= 3 & abs(cor(x, y)) > 0.99999,
         x = ifelse(needs_jitter, jitter(x, amount = 0.00075), x),
         y = ifelse(needs_jitter, jitter(y, amount = 0.00075), y) ) %>%
  select(-needs_jitter) %>%
  ungroup()


p.Taouk_Disc_network_with_ellipses <- genomic.sites.network.gg_gono %>% 
  mutate(region.cluster.present=ifelse(region.cluster %in% Taouk_regions.retained,"present","missing")) %>%
  mutate(region.cluster.filt=ifelse(region.cluster %in% Taouk_regions.retained, region.cluster, NA)) %>%
  ggplot( aes(x = x, y = y, xend = xend, yend = yend)) + 
  geom_edges(alpha=0.25,curvature = 0, color='grey25') + 
  #geom_nodes(size=2, alpha=0.5, aes(color=region.cluster)) +
  geom_nodes(size=1.5, alpha=0.5, aes(color=population)) +
  scale_color_manual(breaks=Taouk_clusters.cols$cluster, values=Taouk_clusters.cols$cluster.cols) +
  new_scale_color() +
  theme_blank() + theme(legend.position='none') +
  #geom_mark_ellipse(aes(group=region.cluster.filt, color=region.cluster.present), expand=0.0025, linewidth=0.2, alpha=0.7) +
  geom_mark_ellipse(data = ellipse_data, aes(group=region.cluster.filt, color=region.cluster.present), expand=0.0025, linewidth=0.2, alpha=0.7) +
  scale_color_manual(breaks=c("missing","present"), values=c("transparent", "red"), guide = 'none') +
  theme(legend.position='none') +
  NULL
#p.Taouk_Disc_network_with_ellipses
```

Create a combination plot
```{r, fig.height=12, fig.width=10}
p.Taouk_Disc_network_with_ellipses_and_support <- plot_grid(p.Taouk_Disc_network_with_ellipses, p.Taouk_gono_pop_support_bars__before.after, ncol=1, rel_heights = c(3,1) , labels="AUTO")

p.Taouk_Disc_network_with_ellipses_and_support



#ggsave(plot=p.Taouk_Disc_network_with_ellipses_and_support, paste0(Figures_directory,"SuppFig14__Gono_SNP-network.",format(Sys.Date(),"%Y%m%d"),".pdf"), units='mm', width=220, height=320, device='pdf', dpi=800)
```


\
Ok, now extract the sequences from those regions and make a tree. 
\
Need to remake function to be more generic
```{r}
Taouk_gono.FST.sig_single.sublin.sites_region.phycon.boundaries_retained


Taouk.insert.sites__goodregions.IRanges <- IRanges(start=Taouk_gono.FST.sig_single.sublin.sites_region.phycon.boundaries_retained$start.extra, end=Taouk_gono.FST.sig_single.sublin.sites_region.phycon.boundaries_retained$end.extra, names=Taouk_gono.FST.sig_single.sublin.sites_region.phycon.boundaries_retained$region.cluster)
...
# Function to find sites within amplicons
extract_SNPs_in_amplicons2 <- function(my.SNPsites, my.phycon.regions){
  
  # #testing
   #my.SNPsites <- as.numeric(colnames(Taouk_gono.processed.vcf)[2:length(colnames(Taouk_gono.processed.vcf))])
   #my.phycon.regions <- Taouk_gono.FST.sig_single.sublin.sites_region.phycon.boundaries_retained
  
  # Create IRanges object from amplicons
    Phycon.sites.IRanges <- IRanges(start=as.numeric(my.phycon.regions$start.extra), end=as.numeric(my.phycon.regions$end.extra), names=as.character(my.phycon.regions$region.cluster))
  # Create IRanges object from sites
  current.SNPsites.IRanges <- IRanges(start=as.numeric(my.SNPsites), end=as.numeric(my.SNPsites), name=as.character(my.SNPsites))
  # test if there are any intersects - if there are, run a bunch of analyses, if there are not, generate an empty file
  if(nrow(data.frame(IRanges::findOverlaps(current.SNPsites.IRanges, Phycon.sites.IRanges, select="all"))) >=1){
    # find overlaps with primer scheme amplicons
    current.SNPsites__IRanges_overlap_with_phycons <- data.frame(IRanges::findOverlaps(current.SNPsites.IRanges, Phycon.sites.IRanges, select="all"))
    # Add amplicon overlap names
    current.SNPsites__IRanges_overlap_with_phycons$amplicons <- sapply(1:length(current.SNPsites__IRanges_overlap_with_phycons$subjectHits), function(x) my.phycon.regions$region.cluster[current.SNPsites__IRanges_overlap_with_phycons$subjectHits[x]])
    # add SNP positions
    current.SNPsites__IRanges_overlap_with_phycons$SNP_Pos <- sapply(1:length(current.SNPsites__IRanges_overlap_with_phycons$queryHits), function(x) my.SNPsites[current.SNPsites__IRanges_overlap_with_phycons$queryHits[x]])
    current.SNPsites__IRanges_overlap_with_phycons$true.snp <- 1
  }
  else {
    current.SNPsites__IRanges_overlap_with_phycons <- data.frame(queryHits=0, subjectHits=0, amplicons=0, SNP_Pos=0, true.snp=0)
  }
  return(current.SNPsites__IRanges_overlap_with_phycons)
}
```
\

\
Ok, want to make a multiple sequence alignment from the Phy-cons (derived from the original WGS alignment)
```{r}
Taouk_gono.processed_all.SNPs <- colnames(Taouk_gono.processed.vcf)[2:length(colnames(Taouk_gono.processed.vcf))]

Taouk_gono.processed_amplicon_SNPs <- extract_SNPs_in_amplicons2(my.SNPsites = Taouk_gono.processed_all.SNPs, 
                           my.phycon.regions = Taouk_gono.FST.sig_single.sublin.sites_region.phycon.boundaries_retained)

Taouk_gono.processed_amplicon_SNPs

Taouk_gono.processed_amplicon_SNPs.list <- unique(Taouk_gono.processed_amplicon_SNPs$SNP_Pos)
Taouk_gono.processed_amplicon_SNPs.list

length(Taouk_gono.processed_amplicon_SNPs.list)
...
Taouk_snps_df <- data.frame(position = as.numeric(Taouk_gono.processed_amplicon_SNPs.list))

# Set genome and binning parameters
Taouk_window_size <- 350  # bin size in bp
Taouk_genome_length <- max(Taouk_snps_df$position)
Taouk_bin_edges <- seq(0, Taouk_genome_length + Taouk_window_size, by = Taouk_window_size)

# Assign SNPs to bins
Taouk_snps_df <- Taouk_snps_df %>%
  mutate(bin_id = findInterval(position, Taouk_bin_edges))

# Count SNPs per bin (use dplyr::count to avoid masked function issues)
snp_counts <- dplyr::count(Taouk_snps_df, bin_id)

# Create full list of bins
Taouk_bins <- tibble(bin_id = seq_along(Taouk_bin_edges[-1]), bin_start = Taouk_bin_edges[-length(Taouk_bin_edges)], bin_end = Taouk_bin_edges[-1] - 1 )

# Join SNP counts to bins, fill in bins with 0 SNPs
Taouk_phycon.bin_counts <- dplyr::right_join(snp_counts, Taouk_bins, by = "bin_id") %>%
  mutate(n = tidyr::replace_na(n, 0)) %>%
  arrange(bin_id)

# Apply rolling mean to SNP counts
Taouk_phycon.bin_counts <- Taouk_phycon.bin_counts %>%
  mutate(rolling_mean = roll_mean(n, n = 5, fill = NA, align = "center")) %>%
  mutate(bin_centre = bin_start + Taouk_window_size/2) %>%
  mutate(rolling_density = rolling_mean/Taouk_window_size)

# Plot
Taouk_phycon.bin_counts %>%
  ggplot(aes(x=bin_centre, y=rolling_density)) +
  geom_point(alpha=0.5) +
  theme_bw() + theme.text.size + x.theme.axis.rotate.angle +
  labs(y="SNP density (vars/window)", x=paste0("Genome Position (", Taouk_window_size, "bp windows)"))
```

\
```{r, eval=F}
Taouk_gono.processed.vcf.melt_in.phycons <- Taouk_gono.processed.vcf.melt %>% 
  filter(POS %in% Taouk_gono.processed_amplicon_SNPs.list)

Taouk_gono.processed.vcf.melt_in.phycons  
...
# 
# Taouk_gono.processed.vcf.melt %>%
#   mutate(POS2 = as.numeric(POS))
```




???SNP files are too large to process on a laptop (memory limitations). 
Could use bedtools to extract from the MSA?
\

\
# Code to extract phycon sites from a bigger table
filter_tsv_to_phycon_sites.R
```{r, eval=F}
#!/usr/bin/env Rscript
...
SNP_table.processed.vcf <- read.csv(SNP_table.file, check.names = F)
...
position_list <- read.csv(position_list.file, check.names = F, header=F)
...
\
read in results of above script (run on cluster)
```{r}

# Toy example for interactive compute
Taouk_gono.processed.vcf.melt_in.phycons <- read.csv(Taouk_gono.processed.vcf.melt_in.phycons.file, check.names = F, header=T)
...
Taouk_gono.processed.vcf.melt_in.phycons %>%
  ...
  group_by(seq) %>%
  summarise(seqcount = n())

Taouk_gono.processed.vcf.melt_in.phycons_seqs <- Taouk_gono.processed.vcf.melt_in.phycons %>%
  mutate(gt_GT_alleles = ifelse(gt_GT_alleles == "*","N", gt_GT_alleles)) %>%
  group_by(Indiv) %>%
  summarise(seq = paste(gt_GT_alleles, collapse=""))
Taouk_gono.processed.vcf.melt_in.phycons_seqs



# # Create file name for fasta output
# Taouk_gono.SNPs_in.phycons.fasta.filename <- paste0("/Users/mb29/Papers/AmpliSeq_Treponema_paper_2024/Github/TP-Phylo-Plex_paper_2025/Gono_data/","Taouk_gono.SNPs_in.phycons.fa")
# 
# for (current.sample in c(1:length(Taouk_gono.processed.vcf.melt_in.phycons_seqs$Indiv))){
#   seqinr::write.fasta(names=Taouk_gono.processed.vcf.melt_in.phycons_seqs$Indiv[current.sample], sequences = Taouk_gono.processed.vcf.melt_in.phycons_seqs$seq[current.sample], file.out = Taouk_gono.SNPs_in.phycons.fasta.filename, open="a", as.string = T)
# }
...
\
Read in trees and plot
```{r, fig.width=8, fig.height=8}
Gono_Phycon_tree.phylo <- midpoint.root(read.tree(Gono_Phycon_tree.file))

Gono_Phycon_tree.rect.gg <- ggtree(Gono_Phycon_tree.phylo, size=0.25, right=T)

# Gono_Phycon_tree.rect.gg
```
\
Define metadata
```{r}
Taouk_gono.main_clusters <- Taouk_gono.metadata %>%
  filter(cluster_limited!="Other") %>%
  select(Accession, cluster_limited)
```
...
# Gono_Phycon_tree.circ.gg %<+% Taouk_gono.main_clusters + 
#   geom_tippoint(aes(color=cluster_limited), size=1, alpha=0.35) +
#   scale_color_manual(breaks=Taouk_clusters.cols$cluster, values=Taouk_clusters.cols$cluster.cols)

Gono_Phycon_tree.rect.gg %<+% Taouk_gono.main_clusters + 
  geom_tippoint(aes(color=cluster_limited), size=1, alpha=0.35) +
  scale_color_manual(breaks=Taouk_clusters.cols$cluster, values=Taouk_clusters.cols$cluster.cols)
```

\
Do the same for the WGS tree

```{r, fig.width=10, fig.height=10}
Gono_WGS_tree.phylo <- midpoint.root(read.tree(Gono_WGS_tree.file))

Gono_WGS_tree.rect.gg <- ggtree(Gono_WGS_tree.phylo, size=0.25, right=T)

Gono_WGS_tree.rect.gg %<+% Taouk_gono.main_clusters + 
  geom_tippoint(aes(color=cluster_limited), size=1, alpha=0.35) +
  scale_color_manual(breaks=Taouk_clusters.cols$cluster, values=Taouk_clusters.cols$cluster.cols)
```
...
Now do a tanglegram
```{r, fig.width=10, fig.height=10}
p.Taouk_WGS.vs.Phycon_Tanglegram <- plot_tanglegram(Gono_WGS_tree.phylo, Gono_Phycon_tree.phylo, data.frame(Sample_Name=Taouk_gono.main_clusters$Accession, Lineage=Taouk_gono.main_clusters$cluster_limited), colorscheme = Taouk_clusters.cols, scale_difference = 0.0075, centre_separation = 0.001, pointsize = 1, left_label_offset = 0.05, right_label_offset = 1, left_ladder = T, right_ladder = T, line_alpha = 0.15, untangle=F) + theme(legend.position = 'bottom') + guides(col = guide_legend(nrow = 7))

p.Taouk_WGS.vs.Phycon_Tanglegram 

#ggsave(plot=p.Taouk_WGS.vs.Phycon_Tanglegram, paste0(Figures_directory,"SuppFig15__Gono_Tanglegram.",format(Sys.Date(),"%Y%m%d"),".pdf"), units='mm', width=220, height=220, device='pdf', dpi=800)
```
\
untangled
```{r, fig.width=10, fig.height=10}
plot_tanglegram(Gono_WGS_tree.phylo, Gono_Phycon_tree.phylo, data.frame(Sample_Name=Taouk_gono.main_clusters$Accession, Lineage=Taouk_gono.main_clusters$cluster_limited), colorscheme = Taouk_clusters.cols, scale_difference = 0.0075, centre_separation = 0.001, pointsize = 1, left_label_offset = 0.05, right_label_offset = 1, left_ladder = F, right_ladder = F, line_alpha = 0.15, untangle=T) + theme(legend.position = 'bottom') + guides(col = guide_legend(nrow = 7))
```

################
\
\
Print all plots (for drafting purposes)
```{r, eval=F}
# Read the notebook as text
rscript_code <- readLines("Treponema_Phylo-Seq_Discrimintory_amplicon_analysis_04-2025.Rmd")

# Extract lines containing ggsave
ggsave_lines <- grep("ggsave", rscript_code, value = TRUE)
ggsave_lines <- gsub("\\#\\ *","", ggsave_lines)

# Evaluate each one
for (line in ggsave_lines) {
